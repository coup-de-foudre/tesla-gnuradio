...
RUN apt-get install -y git subversion axel wget zip unzip \
  ...
  libfftw3-dev libboost-all-dev libcppunit-dev libgsl0-dev libusb-dev \
  libsdl1.2-dev python-wxgtk3.0 python-numpy python-cheetah python-lxml \
  doxygen libxi-dev python-sip libqt4-opengl-dev libqwt-dev libfontconfig1-dev \
...
COPY scripts/build-gnuradio .
RUN chmod a+x ./build-gnuradio && printf "y\ny\ny\ny\n" | ./build-gnuradio -ja -e all
